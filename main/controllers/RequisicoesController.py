...
class RequisicoesController(View):
    ...
    def init_requisicoes(self):
        ...
        for key_municipio_id in clientes_by_municipio_id:
            ...
            logging.debug("Fazendo requisições para " + municipio['nome'] + " - " + estado['sigla'])
    ...
    def verifica_condicoes_perigosas(self, chuvas_iuv, ondas, condicoes_tempo) -> dict:
        ...
        if(self.existe_dados(chuvas_iuv)):
            resposta['chuvas'] = self.verifica_chuvas(chuvas_iuv, condicoes_tempo)
            resposta['iuv'] = self.verifica_iuv(chuvas_iuv)
        else:
            logging.warning("Não há dados de chuvas nem iuv, não é possível verificar se são perigosos!")
        
        if(self.existe_dados(ondas)):
            resposta['ondas'] = self.verifica_ondas(ondas)
        else:
            resposta['ondas'] = {'manha': None, 'tarde': None, 'noite': None}
            logging.info("Não há dados de ondas, não é possível verificar se são perigosos")
    ...
    def verifica_chuvas(self, chuvas_iuv, condicoes_tempo) -> dict:
        ...
        if(not self.verifica_data(previsao.dia)):
            logging.info("Não foi encontrada previsão de chuvas para o dia de amanhã. Previsão mais próxima para: " + previsao.dia)
            return None
        ...
        logging.info("Nenhuma condição perigosa de chuvas encontrada. Previsão: " + descricao)
    ...
    def verifica_iuv(self, chuvas_iuv):
        ...
        #checa data
        if(not self.verifica_data(previsao.dia)):
            logging.info("Não foi encontrada previsão de iuv para o dia de amanhã. Previsão mais próxima para: " + previsao.dia)
            return None     
        ...
        if(previsao_iuv >= CONDICAO_ALTA_IUV and previsao_iuv < CONDICAO_MUITO_ALTA_IUV):
            ...
            resposta['envia_sms'] = False
            logging.info("Nenhuma previsão perigosa de índices ultra-violeta. iuv: " + previsao.iuv)
            return resposta
    ...
    def verifica_ondas_turno(self, ondas, turno):
        ...
        data_previsao_formatada = dt.datetime.strptime(data_previsao, "%d-%m-%Y").strftime("%Y-%m-%d")
        ...
        if(ondas.agitacao == "Fraco" or ondas.agitacao == "Moderado"):
            logging.info("Sem necessidade de enviar alerta, mar: " + ondas.agitacao)
            resposta['envia_sms'] = False
        elif(ondas.agitacao == "Forte"):
            logging.info("Condição perigosa de ondas encontrada, mar: " + ondas.agitacao)
            resposta['envia_sms'] = True
            resposta['descricao_condicao_perigosa'] = ondas.agitacao
        else:
            logging.warning("Não foi possível detectar previsão da agitação das ondas")
            resposta['envia_sms'] = False
    ...
    def verifica_dados_sms(self, dados, clientes):
        ...
        dict_sms = self.buffer_sms(dados['ondas']['manha'], 'ondas-manha', dict_sms)
        dict_sms = self.buffer_sms(dados['ondas']['tarde'], 'ondas-tarde', dict_sms)
        dict_sms = self.buffer_sms(dados['ondas']['noite'], 'ondas-noite', dict_sms)
    ...
    def buffer_sms(self, dado, tipo, dict_sms):
        if(dado!= None):
            ...
            logging.info("Dados de " + tipo + " não obtidos, não haverá envio de sms")
    ...
    def verifica_data(self, dia):
        ...
        if(dia != data_amanha_formatada): return False
        return True
