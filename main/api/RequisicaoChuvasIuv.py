        ...
        for elem in root:
            ...
            if elem.tag == "previsao":
                for e in elem:
                    ...
                    if e.tag == "tempo": tempo = e.text
                    if e.tag == "maxima": maxima = e.text
                    if e.tag == "minima": minima = e.text
                    if e.tag == "iuv": iuv = e.text
                p = Previsao(dia, tempo, maxima, minima, iuv)
                listaPrevisao.append(p)
        self.pv_chuvas_iuv = PrevisaoChuvas(nome, uf, atualizacao, listaPrevisao) 
