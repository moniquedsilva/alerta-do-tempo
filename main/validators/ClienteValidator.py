...
QTD_DIG_CEL = 9
DDI_BRASIL = "55"
DDDS_VALIDOS = ["11", "12", "13", "14", "15", "16", "17",
                "18", "19", "21", "22", "24", "27", "28", "31", "32", "33",
                "34", "35", "37", "38", "41", "42", "43", "44", "45", "46",
                "47", "48", "49", "51", "53", "54", "55", "61", "62", "63",
...
class ClienteValidator:
    ...
    def valida(self):
        ...
        if(not resposta_valida_nome['status']):
            return resposta_valida_nome

        resposta_valida_ddi = self.valida_ddi()
        if(not resposta_valida_ddi['status']):
            return resposta_valida_ddi

        resposta_valida_ddd = self.valida_ddd()
        if(not resposta_valida_ddd['status']):
            return resposta_valida_ddd

        resposta_valida_celular = self.valida_celular()
        if(not resposta_valida_celular['status']):
            return resposta_valida_celular

        resposta_valida_senha = self.valida_senha()
        if(not resposta_valida_senha['status']):
            return resposta_valida_senha

        resposta_valida_estado_id = self.valida_estado_id()
        if(not resposta_valida_estado_id['status']):
            return resposta_valida_estado_id

        resposta_valida_municipio_id = self.valida_municipio_id()
        if(not resposta_valida_municipio_id['status']):
            return resposta_valida_municipio_id
    ...
    def valida_nome(self):
        ...
        if (not match):
            return {'status': False, 'msg': "Não é permitido números nem caractéres especiais no nome"}
    ...
    def valida_ddi(self):
        ...
        if(ddi != DDI_BRASIL):
            return {'status': False, 'msg': "Apenas permitimos ddi do Brasil. e.g: 55"}
        return {'status': True}
    ...
    def valida_ddd(self):
        ...
        if(len(ddd) != QTD_DIG_DDD):
            return {'status': False, 'msg': "O DDD deve conter " + QTD_DIG_DDD + "!"}
        # checa se ddd válido

        if(DDDS_VALIDOS.count(ddd) <= 0):
            return {'status': False, 'msg': "DDD inválido"}
    ...
    def valida_celular(self):
        ...
        if (not match):
            return {'status': False, 'msg': "Apenas dígitos numéricos são permitidos no celular. Ex: 98844332211"}

        # primeiro dígito deve ser 9
        if (celular[0] != PRIMEIRO_DIGITO_CEL):
            return {'status': False, 'msg': "O primeiro dígito do celular deve ser " + PRIMEIRO_DIGITO_CEL}
    ...
    def valida_senha(self):
        ...
        if(len(senha) < QTD_MIN_DIG_SENHA):
            return {'status': False, 'msg': "Digite uma senha com mais de " + str(QTD_MIN_DIG_SENHA) + "!"}
